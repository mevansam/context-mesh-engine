...
`client_credentials` writes a space-separated `scope` claim. Omit `scope` to get all demo scopes (`tools:list pets:read pets:write orders:create`). Unknown scopes are `invalid_scope`. CORS is enabled on `/oauth/token` so Swagger UI on `:8080` can call this origin.
...
```bash
# application token (all demo scopes)
