...
| `{APIPrefix}/tools/{planId}/…`, `{APIPrefix}/openapi/{planId}` | JSON REST | registered only when `Options.ArazzoLoaders` is non-empty |
| `{APIPrefix}/openapi` | JSON REST | always; catalog OAS (`GET /tools` + `$ref` to latest plan specs) |
