...
1. `engine.New` applies defaults, constructs `mcpgw.Gateway` and `apiv1.Router`, registers health and tools. `GET /openapi` (catalog index) is always registered (`PlansController`, catalog may be nil). If `ArazzoLoaders` is non-empty: validate tool-doc templates, `plans.Load`, optional `PolicyCache` from `PolicyLoader`, `plans.RegisterMCP` (help cache + `tools/list` middleware), register plan execute + per-plan OpenAPI on the same `PlansController`. Any of those errors fail `New`. Help `Lookup` and policy `Load` are not called at `New`.
