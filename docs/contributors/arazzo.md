...
`Run` rejects caller keys that are not on the stripped workflow input schema (`ErrUnexpectedInputs`), including `policyHints` and `secrets`. REST-bound `x-source` keys in the JSON body are also `ErrUnexpectedInputs`. Missing required REST/HTTP params are `ErrMissingInput`. Catalog `POST /plans/query` `data` stays an open object in OpenAPI; after match, `Run` applies the same closed check and REST bind. Invalid JSON body → 400 `invalid json body`. Decoder cap 1 MiB.
